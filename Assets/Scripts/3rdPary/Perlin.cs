...
public static class Perlin
{
    ...
    public static float Noise(float x)
    {
        ...
        x -= Mathf.Floor(x);
        var u = Fade(x);
        return Mathf.Clamp01((1 +Lerp(u, Grad(perm[X], x), Grad(perm[X+1], x-1)) * 2)*.5f);
    }
    ...
    public static float Noise(float x, float y)
    {
        ...
        return Mathf.Clamp01((1 + Lerp(v, Lerp(u, Grad(perm[A  ], x, y  ), Grad(perm[B  ], x-1, y  )),
                       Lerp(u, Grad(perm[A+1], x, y-1), Grad(perm[B+1], x-1, y-1))))*.5f);
    }
    ...
    public static float Noise(float x, float y, float z)
    {
        ...
        return Mathf.Clamp01((1 + Lerp(w, Lerp(v, Lerp(u, Grad(perm[AA  ], x, y  , z  ), Grad(perm[BA  ], x-1, y  , z  )),
                               Lerp(u, Grad(perm[AB  ], x, y-1, z  ), Grad(perm[BB  ], x-1, y-1, z  ))),
                       Lerp(v, Lerp(u, Grad(perm[AA+1], x, y  , z-1), Grad(perm[BA+1], x-1, y  , z-1)),
                               Lerp(u, Grad(perm[AB+1], x, y-1, z-1), Grad(perm[BB+1], x-1, y-1, z-1)))))*.5f);
    }
